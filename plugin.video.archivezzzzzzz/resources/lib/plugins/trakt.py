        ...
        if url.startswith("trakt"):
            ...
            if split[1] == "movies":
                ...
                if split[3] == "collection":
                    ...
                    watched = api.get_watched(user_id, split[4], page=page)
                    return api.handle_list(watched, page_link=page_split[0] + "|" + str(page + 1))
...
class Trakt_API:
    ...
    def get_watched(self, user_id: str, type: str, page: int = 1):
        response = self.session.get(f"{self.base_url}/users/{user_id}/watched/{type}?extended=full", headers=self.headers, params={"page": page, "limit": 25})
